//
//  Socket.cpp: TCP/IP or datagram INET sockets.
...
// type is usually SOCK_STREAM (TCP/IP) or SOCK_DGRAM (datagram connection)
// protocol is 0 (meaning it is chosen automatically)
Socket::Socket(int _type) :
sockfd(::socket(AF_INET, _type, /*protocol*/0)) {
}

// private constructor for ServerSocket.
// first arg is not used, only serves for ServerSocket::accept()
Socket::Socket(ServerSocket*, int _sockfd) : sockfd(_sockfd) {}
...
struct sockaddr* Socket::initIN4Address(struct sockaddr_in& addr,
                                        const string& host, int port) {
  struct hostent* hostinfo = NULL;
  // gethostbyname() is obsolete!
  if (host.empty() || !(hostinfo = ::gethostbyname(host.c_str()))) return NULL;
...
int Socket::bind(const string& host, int port) {
  ...
  if (initIN4Address(addr, host, port) == NULL) return UNKNOWN_HOST;
...
//int ServerSocket::bind(int port, int backlog, bool reuse_address) {
int ServerSocket::bind(int port, int backlog) {
  ...
  //if (stat < 0 && !reuse_address) return stat;
...
Socket* ServerSocket::accept() {
  ...
  if ((sock_com = ::accept(sockfd, NULL, NULL)) < 0)
    ...
    return new Socket(this, sock_com);
}
...
ssize_t SocketBuffer::readLine(char* str, size_t num, bool& truncated) {
  ...
  while (true) {
    ...
    if (*pdelim == '\n' || *pdelim == '\r') {
      *pdelim = '\0';   // délimiteur trouvé => le remplacer par nul terminator
      saveRemainings(pdelim+1, received-(pdelim-str+1), remaining, remaining_size);
      return pdelim-str+1;
    }
    else pcurrent += received;   // preparer l'iteration suivante

    // délim pas trouvé, le buffer n'est pas assez grand
    if (pcurrent >= pend) {
      truncated = true;
      *pend = '\0';
      saveRemainings(NULL, 0, remaining, remaining_size);
      return pend - str;
    }
  }
}
...
void SocketBuffer::saveRemainings(const char* buf1, size_t size1, const char* buf2, size_t size2) {
